...
class Film extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return this.state.activeTab !== nextState.activeTab || !Immutable.is(this.props.allFilmList, nextProps.allFilmList) || !Immutable.is(this.props.popularityFilmList, nextProps.popularityFilmList);
  }
  ...
  switchTab = (type) => {
    return (event) => {
      ...
      //切换之前先清空
      filmAction.cleanFilmList(this.state.type);

      this.setState({
        activeTab: type
      });

      filmAction.getFilmList(type);
    };
  };
